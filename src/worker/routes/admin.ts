...
const appSchema = z.object({
  ...
  // 영문 문구 — 비워 두면 화면에서 한글 값을 그대로 쓴다
  nameEn: z.string().max(80).optional().nullable().or(z.literal("")),
  taglineEn: z.string().max(120).optional().nullable().or(z.literal("")),
  descriptionEn: z.string().max(4000).optional().nullable().or(z.literal("")),
...
adminRoutes.put("/apps/:id", requireRole("crew"), async (c) => {
  ...
  const updated = await db
    ...
    .set({
      ...
      ...(d.nameEn !== undefined && { nameEn: d.nameEn || null }),
      ...(d.taglineEn !== undefined && { taglineEn: d.taglineEn || null }),
