#!/usr/bin/env python3
"""BookViewer 앱 아이콘 — BRAND.md 의 캐릭터 EXA(북뷰 변형) 기준.

`assets/brand/svg/appicon-bookview.svg` 를 그대로 래스터화한다. SVG 렌더러를
...
수치를 바꾸면 EXA 가 아니게 되므로(BRAND.md §8) 여기 상수는 임의로 손대지 않는다.
    ...
    동공(신호)        cx=32 cy=32 r=4.5           ← 북뷰는 앰버
...
OUT = os.path.join(ROOT, "apps", "bookviewer", "design", "brand")
SS = 4
N = 1024 * SS
...
def draw_book_hint(layer, s=1.0, cx=32.0, cy=32.0):
    """하단 펼친 책 실루엣 — 북뷰 제품 변형의 글리프 (BRAND.md §2.4).
